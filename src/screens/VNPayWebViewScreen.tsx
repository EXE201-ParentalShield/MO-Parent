...
const VNPayWebViewScreen = ({ navigation, route }: VNPayWebViewScreenProps) => {
  const { paymentUrl, paymentId, token, packageName } = route.params;
  ...
  const handleNavigationStateChange = (navState: WebViewNavigation) => {
    ...
    // Bắt deep link của app (vnpay-return hoặc payment-result)
    ...
    if (
      ...
      // Parse query params từ URL redirect
      ...
      // Nếu VNPay redirect thành công → bắt đầu polling để lấy kết quả chính thức
      startPolling();
    }
  };
    ...
    <View style={styles.container}>
      ...
      {!hasError && (
        <WebView
          ...
          // Cho phép giả lập mobile để VNPay hiển thị đúng
